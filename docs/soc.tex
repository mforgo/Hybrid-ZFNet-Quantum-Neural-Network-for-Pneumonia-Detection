...
Optimalizace navrženého řešení probíhala ve dvou na sobě nezávislých krocích: (i) trénink kvantového klasifikátoru VQC v rámci hybridního modelu s fixovanou klasickou částí (ResNet-50 + PCA pipeline) a (ii) trénink čistě klasického baseline klasifikátoru nad stejnými 64rozměrnými příznaky. V obou případech byl použit algoritmus \textbf{Adam}, avšak s odlišným nastavením learning rate, plánů učení a chybových funkcí reflektujících rozdílný charakter výstupu kvantového obvodu a sigmoidového klasického modelu.
...
\begin{itemize}
    ...
    \]
    která je následně interpretována jako skóre pneumonie po lineárním přemapování na interval \([0,1]\) až ve fázi vyhodnocení. Aby bylo možné přímo optimalizovat kvantové parametry na výstupu v intervalu \([-1,1]\), jsou binární labely \(y_i \in \{0,1\}\) nejprve převedeny na cílové hodnoty
    \[
        t_i = 
        \begin{cases}
            -1, & y_i = 0 \ (\text{Normal}),\\
    ...
    \]
    Váhy \(w(t_i)\) jsou odvozeny z poměru tříd v trénovací sadě tak, aby byla \emph{důrazněji penalizována převaha třídy Pneumonia} a zohledněna výrazná nevyváženost dat (\(\approx 74{,}2\,\%\) snímků s pneumonií). Tato volba MSE (namísto například BCE) je motivována plynulými gradienty v prostoru parametrů ansatzu a dobrou podporou v autograd backendu knihovny PennyLane.

    \item \textbf{Klasický baseline: vážená Binary Cross-Entropy (BCE)}

    Čistě klasický baseline model nad 64rozměrnými příznaky z PCA vrací pro každý vzorek pravděpodobnost pneumonie \(\hat{p}_i \in [0,1]\) pomocí sigmoidové výstupní neurony. Pro trénink je použita vážená binární cross–entropie
    \[
        \mathcal{L}_{\text{WBCE}}
        = -\frac{1}{N} \sum_{i=1}^N w(y_i)\,\Bigl[\,y_i \log(\hat{p}_i) 
        + \bigl(1-y_i\bigr)\log\bigl(1-\hat{p}_i\bigr)\Bigr],
    \]
    kde funkce \(w(y_i)\) nastavuje vyšší váhu vzorkům třídy \textit{Normal} tak, aby se zmírnil vliv nevyváženosti a model nebyl příliš „přesvědčen“ o přítomnosti pneumonie. V implementaci byla zvolena jednoduchá schémata vah \(w(0) > w(1)\), což vede ke snížení počtu falešně negativních případů pneumonie při zachování rozumné specificity.
\end{itemize}

Na rozdíl od původního návrhu není v této práci minimalizována jediná sdílená kombinovaná ztráta \(\mathcal{L} = \alpha \mathcal{L}_{\text{QMSE}} + (1-\alpha)\mathcal{L}_{\text{WBCE}}\), ale kvantový a klasický model jsou optimalizovány \emph{odděleně} se svými vlastními chybovými funkcemi a hyperparametry.

\subsubsection{Optimalizátory a learning rates}

V obou experimentech je použit optimalizační algoritmus \textbf{Adam} se standardním nastavením \(\beta_1 = 0{,}9\), \(\beta_2 = 0{,}999\) a \(\epsilon = 10^{-8}\). Learning rate a případná regulace jsou voleny odlišně pro kvantový ansatz a klasický baseline klasifikátor:
...
Vyšší learning rate pro kvantový ansatz (\(0{,}01\)) reflektuje menší velikost gradientů v prostoru kvantových parametrů a potřebu rychlejší explorace parametrického prostoru při zachování stability tréninku. U klasického baseline modelu se naopak osvědčilo konzervativnější nastavení \(1 \times 10^{-3}\), které poskytlo stabilní konvergenci bez oscilací validační ztráty.

\subsubsection{Tréninkový režim a plán učení}

Oba modely byly trénovány s mini–batch velikostí \(\text{batch size} = 16\) a maximálním počtem \(50\) epoch, přičemž byla využita kombinace \textbf{early stopping} a \textbf{cosine annealing} plánu učení.

\begin{itemize}
    \item \textbf{Early stopping:} Pro kvantový i klasický model se sledovala validační ztráta s \emph{patience} \(= 10\) epoch. Pokud nedošlo k jejímu zlepšení v daném intervalu, trénink se ukončil a pro další vyhodnocení byly použity parametry s nejnižší dosaženou validační chybou.
    \item \textbf{Plán učení:} 
    \begin{itemize}
        \item U kvantového modelu je Adam kombinován s vlastním \emph{cosine annealing} plánovačem, který plynule snižuje learning rate z počáteční hodnoty na minimální \(\text{LR}_{\text{min}} = 10^{-5}\), přičemž první tři epochy slouží jako lineární \emph{warmup}.
        \item U klasického baseline modelu je použit plánovač \texttt{CosineAnnealingLR} z knihovny PyTorch s parametrem \(T_{\max} = 50\), který postupně snižuje efektivní learning rate v průběhu tréninku, aniž by byla použita explicitní L2 regularizace (weight decay = 0).
    \end{itemize}
\end{itemize}

Tato kombinace vážených chybových funkcí, rozdílného nastavení learning rate a kosinového plánu učení umožnila jak kvantovému, tak klasickému modelu dosáhnout stabilní konvergence při respektování nevyváženosti datasetu a omezených vpočetních možností při simulaci kvantového obvodu.
