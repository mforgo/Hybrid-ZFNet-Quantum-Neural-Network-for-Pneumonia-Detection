...
Navržený hybridní model kombinuje silné stránky hlubokých konvolučních sítí a variačních kvantových obvodů. Vstupní rentgenový snímek je nejprve zpracován předtrénovanou sítí ResNet-50, která jej převede do vektoru příznaků pevné délky.[file:1][file:2] Tento vektor je následně zredukován na nižší dimenzi a normalizován tak, aby byl vhodný pro zakódování do kvantového stavu na omezeném počtu qubitů. Redukované příznaky jsou amplitudově zakódovány do systému šesti qubitů, na něž je aplikován mělký, lokálně proplétající variační obvod (VQC), jenž plní roli klasifikátoru.[file:1] Výstupem kvantového obvodu je skalární hodnota interpretovaná jako pravděpodobnost pneumonie, na kterou je v závěrečném post-processingu aplikováno hledání optimálního prahu pro binární rozhodnutí. Takto navržená pipeline umožňuje zpracovat vysokorozměrná obrazová data klasickým modelem a přesunout pouze kompaktní reprezentaci do kvantové části, která je přizpůsobena omezeným možnostem současného NISQ hardwaru.[file:1][file:2]

\section{Datový soubor (Dataset)}

Pro trénování a testování modelu byl využit veřejně dostupný dataset \textit{Chest X-Ray Images (Pneumonia)}, který pochází z Guangzhou Women and Children’s Medical Center a je distribuován prostřednictvím platformy Kaggle.[file:1] Dataset obsahuje rentgenové snímky hrudníku (předozadní projekce) pediatrických pacientů ve věku přibližně 1 až 5 let.
...
Celkem dataset zahrnuje 5863 snímků, které jsou v původní verzi rozděleny do trénovací, validační a testovací sady.[file:1] Původní validační sada obsahuje pouze 16 snímků, což se ukázalo jako nedostatečné pro spolehlivé sledování konvergence modelu a ladění hyperparametrů (viz Obrázek~\ref{fig:dataset_distribution}).[file:1]
...
Aby byla validační sada dostatečně reprezentativní, byla v rámci předzpracování provedena reorganizace dat: původní trénovací a validační sada byly sloučeny a nově rozděleny v poměru 80:20 se zachováním stratifikace podle tříd.[file:1] Nové rozdělení má následující velikosti:
\begin{itemize}
    ...
    \item \textbf{Testovací sada:} 624 snímků (ponechána v původním stavu pro finální vyhodnocení).[file:1]
\end{itemize}

Snímky jsou kategorizovány do dvou tříd: \textit{Normal} (zdravý nález) a \textit{Pneumonia} (zápal plic).[file:1] Dataset je značně nevyvážený, přibližně tři čtvrtiny snímků patří do třídy \textit{Pneumonia}, což motivuje použití vyvážené ztrátové funkce a vhodných metrik při vyhodnocování modelu.[file:1] Navíc je patrná vysoká variabilita v kvalitě snímků, rozlišení a přítomnosti artefaktů (např. lékařská zařízení), což dále zvyšuje nároky na robustní předzpracování dat.
...
Před vstupem do neuronové sítě bylo nutné snímky standardizovat tak, aby odpovídaly očekávanému vstupu předtrénované architektury ResNet-50.[file:1] K tomu byla použita pipeline založená na knihovně \texttt{torchvision.transforms}, která provádí následující kroky:

\begin{enumerate}
    \item \textbf{Změna velikosti (Resizing):} Každý snímek je změněn na rozlišení \(224 \times 224\) pixelů, což je standardní vstupní velikost pro ResNet-50.[file:1]
    \item \textbf{Převod na tenzor:} Obraz je převeden na tenzor typu \texttt{float32} a hodnoty intenzit jsou škálovány do intervalu \([0,1]\) pomocí \texttt{ToTensor()}.[file:1]
    \item \textbf{Normalizace podle ImageNet:} Kanály RGB jsou normalizovány pomocí středních hodnot \([0{,}485, 0{,}456, 0{,}406]\) a směrodatných odchylek \([0{,}229, 0{,}224, 0{,}225]\), což odpovídá statistice trénovací množiny ImageNet a zajišťuje, že předtrénovaný ResNet-50 pracuje v očekávaném rozsahu vstupů.[file:1]
\end{enumerate}
...
Jako extraktor příznaků byla zvolena architektura \textbf{ResNet-50}, která představuje standard v oblasti počítačového vidění.[file:1] Síť je použita v režimu \textit{feature extractoru}: poslední klasifikační vrstva je nahrazena identitou a pro každý vstupní snímek je z předposlední vrstvy získán vektor příznaků délky 2048.[file:1]

Každý rentgenový snímek tedy nejprve prochází výše uvedenou obrazovou transformací a poté je zpracován modelem ResNet-50, který jej převede na kompaktní 2048rozměrný vektor.[file:1] Teprve \emph{tyto} vektory jsou následně vstupem do klasické pipeline pro redukci dimenze (PCA) a přípravu dat pro kvantový klasifikátor, nikoli původní obrazová data.[file:1]


\subsection{Redukce dimenzionality: PCA}

Výstupní vektor z ResNet-50 o délce 2048 prvků je pro současné kvantové simulátory a procesory příliš rozměrný.[file:1] Bylo proto nutné jej redukovat na 64 prvků, aby odpovídal Hilbertovu prostoru 6 qubitů (\(2^{6} = 64\)) při amplitudovém kódování.[file:1] V experimentální části byly porovnány tři přístupy: \textbf{PCA}, \textbf{LDA} a \textbf{SelectKBest}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|l|p{8cm}|}
        ...
        \hline
        \textbf{PCA} & Lineární metoda, která hledá směry s největším rozptylem dat a projektuje příznaky do podprostoru s maximální zachovanou variabilitou. V této práci poskytla nejstabilnější trénink a nejlepší validační výsledky při redukci na 64 komponent, a proto byla zvolena jako finální metoda.[file:1] \\
        \hline
        \textbf{LDA} & Supervisovaná metoda hledající projekci maximalizující separaci tříd. V nastavení s vysokou původní dimenzí a omezeným počtem vzorků byla náchylná k numerickým problémům a prakticky nevedla ke spolehlivému zlepšení výsledků, proto nebyla dále použita.[file:1] \\
        \hline
        \textbf{SelectKBest} & Vybírá \(K\) nejrelevantnějších příznaků na základě jejich statistické závislosti na cílové třídě (ANOVA F-test). Ačkoliv zachovává původní interpretovatelné příznaky, v provedených experimentech dosahovala mírně horších validačních metrik než PCA a vedla k méně stabilnímu tréninku kvantového klasifikátoru.[file:1] \\
        \hline
    \end{tabular}
\end{table}

Finální pipeline použitá v této práci proto využívá \textbf{PCA} se 64 komponentami, před kterou je aplikován standardní z-skorovací \textit{StandardScaler} pro centrování a škálování příznaků.[file:1] Výsledné 64rozměrné vektory jsou následně normalizovány na jednotkovou \(L_2\) normu, aby splňovaly požadavek na normu amplitud při amplitudovém kódování do kvantového stavu.[file:1]

\section{Kvantová část: variační kvantový klasifikátor (VQC)}

Kvantová část hybridního modelu je realizována jako variační kvantový klasifikátor (VQC) nad registrem o \(n = 6\) qubitech.[file:1] Každý vzorek je reprezentován 64rozměrným vektorem příznaků získaným z ResNet-50 a následnou redukcí dimenze pomocí PCA, který je nejprve zakódován do kvantového stavu, poté zpracován parametrizovaným kvantovým obvodem (ansatzem) a nakonec je změřen vhodný observábl pro získání skalárního skóre pneumonie.[file:1]

\subsection{Kódování klasických dat do kvantového registru}

Nechť \(\mathbf{x} \in \mathbb{R}^{64}\) označuje vektor příznaků po aplikaci ResNet-50, PCA a následné \(L_2\) normalizaci tak, aby platilo \(\sum_{i=0}^{63} x_i^2 = 1\).[file:1] Takový vektor je možné interpretovat jako amplitudy kvantového stavu na Hilbertově prostoru šesti qubitů:
\[
\lvert \psi(\mathbf{x}) \rangle = \sum_{i=0}^{63} x_i \lvert i \rangle,
\]
kde \(\{\lvert i \rangle\}_{i=0}^{63}\) je výpočetní báze pro 6-qubitový registr.[web:119] V implementaci je toto tzv. \textit{amplitudové kódování} realizováno pomocí šablony \texttt{qml.AmplitudeEmbedding} s parametry \texttt{normalize=True} a \texttt{pad\_with=0.0}, takže je zajištěno splnění normalizační podmínky amplitud.[file:1]

V literatuře se pro kódování klasických vektorů do kvantových stavů běžně používají tři hlavní přístupy: \textit{basis encoding}, \textit{angle encoding} a \textit{amplitude encoding}.[web:116] Basis encoding by v tomto případě vyžadoval 64 qubitů pro přímočaré zakódování indexu vzorku, což je mimo možnosti současného NISQ hardwaru i simulace. Angle encoding by zakódoval jednotlivé složky \(\mathbf{x}\) jako rotační úhly na jednotlivých qubitech (např. pomocí \texttt{AngleEmbedding}), avšak pro 64 příznaků by buď vyžadoval výrazně více qubitů, nebo vysoký počet rotačních hradel a větší hloubku obvodu.[file:1] Amplitudové kódování naopak umožňuje reprezentovat 64rozměrný vektor pomocí pouze šesti qubitů, což exponenciálně šetří qubitové zdroje za cenu složitější přípravné vrstvy, která je však v této práci delegována na backend knihovny PennyLane.[file:1][web:119]
...
\begin{itemize}
    ...
    \item minimalizuje počet parametrizovaných hradel potřebných pro samotný ansatz, protože veškerou „datovou složitost“ nese už vstupní stav.[file:1]
\end{itemize}

\subsection{Parametrizovaný ansatz kvantového obvodu}

Na takto připravený stav je aplikován variační ansatz navržený jako opakování \(L = \texttt{config.n\_layers}\) identických vrstev.[file:1] Každá vrstva se skládá z:
\begin{itemize}
    \item jednoqubitových rotačních hradel \(\mathrm{Rot}(\phi,\theta,\omega)\) na každém qubitu, 
    \item následného kruhového propletení pomocí CNOT hradel mezi sousedními qubity \(w \rightarrow (w+1) \bmod n\).[file:1]
\end{itemize}
...
Pro \(n = 6\) qubitů a \(L\) vrstev má ansatz celkem \(3 \cdot n \cdot L\) volných parametrů, což umožňuje přesně kontrolovat kapacitu modelu skrze volbu hloubky obvodu.[file:1] Lokální rotační operace zajišťují dostatečnou flexibilitu na úrovni jednotlivých qubitů, zatímco kruhové CNOT propletení vytváří pouze lokální entanglement, což je v souladu s doporučeními pro zmírnění barren plateaus a zároveň odpovídá realistické konektivitě mnoha kvantových zařízení.[file:1]

\subsection{Měření a mapování na pravděpodobnost}

Po aplikaci ansatzu je výsledný kvantový stav měřen ve výpočetní bázi prostřednictvím střední hodnoty vybraného observáblu.[file:1] V této práci je kvantový klasifikátor implementován jako QNode, který vrací střední hodnotu operátoru \(Z\) na jednom qubitu,
\[
y_{\text{raw}} = \langle \psi_{\boldsymbol{\theta}}(\mathbf{x}) \rvert Z_k \lvert \psi_{\boldsymbol{\theta}}(\mathbf{x}) \rangle \in [-1, 1],
\]
kde \(\lvert \psi_{\boldsymbol{\theta}}(\mathbf{x}) \rangle\) je výstupní stav po aplikaci ansatzu s parametry \(\boldsymbol{\theta}\).[web:119][file:1] Tato hodnota je následně lineárně přemapována na interval \([0,1]\),
\[
p_{\text{pneumonie}} = \frac{1 + y_{\text{raw}}}{2},
\]
a interpretována jako pravděpodobnost přítomnosti pneumonie pro daný snímek.[file:1]

Ve fázi post-processingu je na tuto spojitou výstupní hodnotu aplikováno tzv. \textit{threshold scan}: hledá se práh \( \tau \in [0,1] \), který maximalizuje zvolenou metriku (např. F1 skóre nebo balanced accuracy) na validační sadě a který se následně použije pro binární rozhodnutí na testovacích datech.[file:1] Tím se odděluje trénink kvantového klasifikátoru (minimalizace ztrátové funkce nad spojitými výstupy) od volby operačního bodu rozhodování, což je v medicínských aplikacích praktické z hlediska ladění kompromisu mezi senzitivitou a specificitou.
...
pneumonie (viz Sekce~\ref{...} o kvantové části). [file:1]
...
\(\hat{y} = \mathbb{1}[p_{\text{pneumonie}} > \tau]\). [file:1]
...
definovaná jako aritmetický průměr \textit{sensitivity} a \textit{specificity}. [file:1]
...
\texttt{test\_probs}. [file:1]
Následně byl pro sadu kandidátních prahů
\(\tau \in \{0.50, 0.55, \dots, 0.90\}\) proveden výpočet následujících metrik: [file:1]
...
\begin{itemize}
    ...
    \(\text{BalAcc} = \tfrac{1}{2}(\text{Sens} + \text{Spec})\). [file:1]
\end{itemize}
...
\(\text{Accuracy}\) a \(\text{Balanced accuracy}\). [file:1]
Implementace tohoto post-processingu v Pythonu je následující:

\begin{lstlisting}[language=Python, caption={Diskrétní optimalizace prahu podle balanced accuracy.}]
y_test = y_data['test']
probs = test_probs
...
pro jednotlivé zvažované hodnoty prahu \(\tau\). [file:1]
...
\(\text{Sensitivity} = 0{,}8000\) a \(\text{Specificity} = 0{,}5769\). [file:1]
...
tak slouží primárně jako analýza citlivosti modelu na volbu rozhodovacího prahu. [file:1]

\subsection{Hyperparametry a optimalizace}

Optimalizace navrženého řešení probíhala ve dvou na sobě nezávislých krocích: (i) trénink kvantového klasifikátoru VQC v rámci hybridního modelu s fixovanou klasickou částí (ResNet-50 + PCA pipeline) a (ii) trénink čistě klasického baseline klasifikátoru nad stejnými 64rozměrnými příznaky.[file:1][file:2] V obou případech byl použit algoritmus \textbf{Adam}, avšak s odlišným nastavením learning rate, plánů učení a chybových funkcí reflektujících rozdílný charakter výstupu kvantového obvodu a sigmoidového klasického modelu.[file:1]
...
\begin{itemize}
    ...
    Kvantový klasifikátor VQC pro každý vzorek vrací očekávanou hodnotu Pauliho operátoru \(Z\) na vybraném qubitu,
    \[
        z_i = \langle Z_0 \rangle_i \in [-1,1],
    \]
    která je následně interpretována jako skóre pneumonie po lineárním přemapování na interval \([0,1]\) až ve fázi vyhodnocení.[file:1] Aby bylo možné přímo optimalizovat kvantové parametry na výstupu v intervalu \([-1,1]\), jsou binární labely \(y_i \in \{0,1\}\) nejprve převedeny na cílové hodnoty
    \[
        t_i = 
        \begin{cases}
            -1, & y_i = 0 \ (\text{Normal}),\\
    ...
    \[
        \mathcal{L}_{\text{QMSE}} 
        = \frac{1}{N} \sum_{i=1}^N w(t_i)\,\bigl(z_i - t_i\bigr)^2.
    \]
    Váhy \(w(t_i)\) jsou odvozeny z poměru tříd v trénovací sadě tak, aby byla \emph{důrazněji penalizována převaha třídy Pneumonia} a zohledněna výrazná nevyváženost dat (\(\approx 74{,}2\,\%\) snímků s pneumonií).[file:1] Tato volba MSE (namísto například BCE) je motivována plynulými gradienty v prostoru parametrů ansatzu a dobrou podporou v autograd backendu knihovny PennyLane.[file:1]

    \item \textbf{Klasický baseline: vážená Binary Cross-Entropy (BCE)}

    Čistě klasický baseline model nad 64rozměrnými příznaky z PCA vrací pro každý vzorek pravděpodobnost pneumonie \(\hat{p}_i \in [0,1]\) pomocí sigmoidové výstupní neurony.[file:1] Pro trénink je použita vážená binární cross–entropie
    \[
        \mathcal{L}_{\text{WBCE}}
        = -\frac{1}{N} \sum_{i=1}^N w(y_i)\,\Bigl[\,y_i \log(\hat{p}_i) 
        + \bigl(1-y_i\bigr)\log\bigl(1-\hat{p}_i\bigr)\Bigr],
    \]
    kde funkce \(w(y_i)\) nastavuje vyšší váhu vzorkům třídy \textit{Normal} tak, aby se zmírnil vliv nevyváženosti a model nebyl příliš „přesvědčen“ o přítomnosti pneumonie.[file:1] V implementaci byla zvolena jednoduchá schémata vah \(w(0) > w(1)\), což vede ke snížení počtu falešně negativních případů pneumonie při zachování rozumné specificity.[file:1][file:2]
\end{itemize}

Na rozdíl od původního návrhu není v této práci minimalizována jediná sdílená kombinovaná ztráta \(\mathcal{L} = \alpha \mathcal{L}_{\text{QMSE}} + (1-\alpha)\mathcal{L}_{\text{WBCE}}\), ale kvantový a klasický model jsou optimalizovány \emph{odděleně} se svými vlastními chybovými funkcemi a hyperparametry.[file:1]

\subsubsection{Optimalizátory a learning rates}

V obou experimentech je použit optimalizační algoritmus \textbf{Adam} se standardním nastavením \(\beta_1 = 0{,}9\), \(\beta_2 = 0{,}999\) a \(\epsilon = 10^{-8}\).[file:1] Learning rate a případná regulace jsou voleny odlišně pro kvantový ansatz a klasický baseline klasifikátor:

\begin{table}[htbp]
\centering
\caption{Nastavení optimalizátorů pro kvantový a klasický model}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Komponenta} & \textbf{Optimizátor} & \textbf{Learning rate} & \textbf{Weight decay} \\
\hline
Klasický baseline (64D MLP) & Adam (PyTorch)     & \(1 \times 10^{-3}\) & \(0.0\) \\
Kvantový ansatz (VQC)       & Adam (PennyLane)   & \(1 \times 10^{-2}\) & \(0.0\) \\
\hline
\end{tabular}
\end{table}

Vyšší learning rate pro kvantový ansatz (\(0{,}01\)) reflektuje menší velikost gradientů v prostoru kvantových parametrů a potřebu rychlejší explorace parametrického prostoru při zachování stability tréninku.[file:1][file:2] U klasického baseline modelu se naopak osvědčilo konzervativnější nastavení \(1 \times 10^{-3}\), které poskytlo stabilní konvergenci bez oscilací validační ztráty.[file:1]

\subsubsection{Tréninkový režim a plán učení}

Oba modely byly trénovány s mini–batch velikostí \(\text{batch size} = 16\) a maximálním počtem \(50\) epoch, přičemž byla využita kombinace \textbf{early stopping} a \textbf{cosine annealing} plánu učení.[file:1]

\begin{itemize}
    \item \textbf{Early stopping:} Pro kvantový i klasický model se sledovala validační ztráta s \emph{patience} \(= 10\) epoch. Pokud nedošlo k jejímu zlepšení v daném intervalu, trénink se ukončil a pro další vyhodnocení byly použity parametry s nejnižší dosaženou validační chybou.[file:1][file:2]
    \item \textbf{Plán učení:} 
    \begin{itemize}
        \item U kvantového modelu je Adam kombinován s vlastním \emph{cosine annealing} plánovačem, který plynule snižuje learning rate z počáteční hodnoty na minimální \(\text{LR}_{\text{min}} = 10^{-5}\), přičemž první tři epochy slouží jako lineární \emph{warmup}.[file:1]
        \item U klasického baseline modelu je použit plánovač \texttt{CosineAnnealingLR} z knihovny PyTorch s parametrem \(T_{\max} = 50\), který postupně snižuje efektivní learning rate v průběhu tréninku, aniž by byla použita explicitní L2 regularizace (weight decay = 0).[file:1]
    \end{itemize}
\end{itemize}

Tato kombinace vážených chybových funkcí, rozdílného nastavení learning rate a kosinového plánu učení umožnila jak kvantovému, tak klasickému modelu dosáhnout stabilní konvergence při respektování nevyváženosti datasetu a omezených vpočetních možností při simulaci kvantového obvodu.[file:1][file:2]
...
Veškeré experimenty byly realizovány v prostředí Jupyter notebooku na platformě Google Colab s Linuxovým operačním systémem a podporou GPU akcelerace přes CUDA 12.6.[file:1][file:2] Jako programovací jazyk byl použit Python ve verzi 3.12.x, přičemž klíčové vědecké a kvantové knihovny byly instalovány pomocí \texttt{pip} na začátku notebooku.[file:1]
...
\begin{itemize}
    \item \textbf{PennyLane} 0.43.2 (\texttt{pennylane}, \texttt{pennylane-lightning-gpu}) pro definici a simulaci variačního kvantového obvodu a integraci s klasickým Python ekosystémem.[file:1][file:2]
    \item \textbf{PyTorch} 2.9.0+cu126 a \textbf{torchvision} 0.24.0+cu126 pro implementaci extraktoru příznaků ResNet-50, klasického baseline klasifikátoru a tréninkové smyčky na GPU.[file:1][file:2]
    \item \textbf{scikit-learn} 1.6.1 pro redukci dimenze (PCA), škálování dat a konstrukci zpracovatelské pipeline.[file:1]
    \item \textbf{NumPy} 2.0.2 a \textbf{SciPy} 1.16.3 pro práci s maticemi, numerické výpočty a pomocné utility.[file:1]
    \item \textbf{pandas} 2.2.2 pro správu metadat a strukturovanou práci s datovými rámci.[file:1]
    \item \textbf{matplotlib} 3.10.0 a \textbf{seaborn} 0.13.2 pro vizualizaci tréninkových křivek a vyhodnocovacích metrik.[file:1]
    \item \textbf{scikit-image} 0.25.2 a \textbf{opencv-python} 4.12.0.88 pro práci s obrazovými daty a pomocné transformace snímků.[file:1]
    \item \textbf{kagglehub} 0.3.13 pro stažení veřejně dostupného datasetu Chest X-Ray Pneumonia z platformy Kaggle.[file:1][file:2]
\end{itemize}

Kvantové obvody byly v této práci simulovány na ideálním backendu \texttt{lightning.gpu} frameworku PennyLane, tj. bez explicitního modelování šumu a dekoherence reálného kvantového hardwaru.[file:2] Nastavení náhodného semene (\texttt{seed = 6}) bylo sjednoceno pro NumPy i PyTorch, aby byla zajištěna reprodukovatelnost všech experimentů v rámci daného softwarového prostředí.[file:1]

\subsubsection{Hardwarové prostředí}

Všechny experimenty byly realizovány v prostředí Google Colab s Linuxovým operačním systémem a GPU akcelerací přes CUDA 12.6.[file:1] Výpočty klasické části (extrakce příznaků ResNet-50, PCA a trénink baseline klasifikátoru) i simulace kvantového obvodu probíhaly na následujících typech GPU:
...
\begin{itemize}
    ...
    \item \textbf{NVIDIA A100} (40\,GB VRAM) -- použita pro plné trénování hybridního modelu a kvantového klasifikátoru při vyšší batch size a delších bězích.[file:1]
\end{itemize}

Kvantová část modelu byla simulována pomocí backendu \texttt{lightning.gpu} z knihovny PennyLane, který využívá CUDA a knihovnu NVIDIA cuQuantum pro akceleraci výpočtu variačních kvantových obvodů na GPU.[file:1] V rámci této práce nebyl použit žádný reálný kvantový procesor; všechny kvantové výpočty probíhaly na klasickém hardwaru formou simulace.
...
\textbf{Interpretace:} Model byl trénován na silně nevyvážených datech (74.2\% pneumonie), ale testován na vyváženějších (62.5\%). Přesto dosáhl 71.63\% accuracy, což demonstruje robustnost.
...
Cílem této práce bylo navrhnout, implementovat a experimentálně ověřit hybridní kvantově–klasický model pro detekci pneumonie z rentgenových snímků hrudníku v podmínkách současné NISQ éry a porovnat jeho výkon s referenčním čistě klasickým modelem založeným na architektuře ResNet-50.[file:2] Součástí cíle byla také analýza vhodných metod redukce dimenzionality, kódování klasických dat do kvantových stavů a návrh variačního kvantového obvodu, který je reálně trénovatelný na omezeném počtu qubitů.[file:2] 

Z metodického hlediska se podařilo vytvořit plně funkční experimentální pipeline, která zahrnuje předzpracování obrazových dat, extrakci příznaků pomocí předtrénované sítě ResNet-50, jejich redukci na rozměr vhodný pro amplitudové kódování a následnou klasifikaci pomocí variačního kvantového obvodu realizovaného v knihovně PennyLane.[file:1][file:2] Během práce bylo nutné upravit původní rozdělení datasetu, neboť validační množina obsahovala pouze 16 snímků; novým stratifikovaným dělením v poměru 80:20 mezi trénovací a validační sadou se podařilo získat robustnější odhad generalizační chyby, aniž by byla narušena integrita testovací množiny.[file:1][file:2]

Experimentální výsledky ukázaly, že klasický model ResNet-50 dosahuje na testovací sadě vyšší přesnosti, F1-skóre i AUC-ROC než navržený hybridní model.[file:1][file:2] Původní hypotéza o dosažení srovnatelné výkonnosti hybridního přístupu v této konkrétní úloze se tak v plné míře nepotvrdila, zejména kvůli omezené expresivitě kvantového ansatzu, redukci vstupního vektoru příznaků a přítomnému dataset shiftu mezi trénovací a testovací množinou.[file:1][file:2] Na druhou stranu hybridní model prokázal, že i relativně mělký variační obvod se 6 qubity a 108 trénovatelnými parametry dokáže dosáhnout smysluplné diskriminační schopnosti (AUC-ROC okolo 0{,}74) v reálné medicínské klasifikační úloze.[file:1][file:2]

Z hlediska praktických přínosů práce lze za nejdůležitější považovat tři oblasti.[file:2] Zaprvé, práce demonstruje kompletní návrh a implementaci hybridní kvantově–klasické architektury na skutečných biomedicínských datech a může tak sloužit jako referenční příklad pro další studenty a výzkumníky v oblasti QML.[file:1][file:2] Zadruhé, systematická analýza omezení – včetně vlivu nevyvážených dat, doménového posunu, volby ansatzu a výpočetních nároků simulace – poskytuje realistický pohled na to, kde se dnes nachází praktická použitelnost kvantového strojového učení v medicíně.[file:1][file:2] Zatřetí, práce otevírá konkrétní cesty pro další výzkum, jako je nasazení na reálný kvantový hardware, využití pokročilých metod mitigace šumu, automatizovaná optimalizace hyperparametrů či zkoumání alternativních kvantových architektur a ensemble přístupů.[file:1][file:2]

Celkově lze říci, že i když hybridní model v této studii nedosáhl lepší klasifikační výkonnosti než pečlivě nastavený klasický ResNet-50, splnil svůj hlavní účel jako experimentální platforma pro zkoumání možností a limitů hybridních kvantově–klasických neuronových sítí v éře NISQ.[file:1][file:2] Výsledky potvrzují, že kvantová výhoda v takto náročné obrazové úloze není za současného stavu hardwaru a při omezeném počtu qubitů samozřejmostí, ale zároveň ukazují, že kvantové obvody mohou hrát roli parametricky úsporných klasifikátorů a inspirují další práci na návrhu architektur, které budou lépe využívat specifické silné stránky kvantových procesorů.[file:1][file:2] V tomto smyslu představuje práce příspěvek k probíhající diskusi o reálné využitelnosti kvantového strojového učení v medicínské diagnostice a vytváří pevný základ pro budoucí experimenty i prohloubení spolupráce mezi oblastí kvantových technologií a klinickou praxí.[file:1][file:2]
