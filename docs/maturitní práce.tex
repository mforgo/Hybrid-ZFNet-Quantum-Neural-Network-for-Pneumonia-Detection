...
\newcommand{\nazevPrace}{Hybridní model pro detekci pneumonie} % 
\newcommand{\podNazev}{Aplikování kvantového strojového učení v~medicínské diagnostice} % 
\newcommand{\typPrace}{Maturitní~práce} % 
\newcommand{\kodPrace}{26\_03} % 
\newcommand{\autor}{Michal Forgó} % 
\newcommand{\trida}{4.F} % 
\newcommand{\skola}{Střední škola informatiky a~finančních služeb, Plzeň, Klatovská 200 G} % 
\newcommand{\konzultant}{Bc. Jan Boháč} % [cite: 388]
\newcommand{\skolniRok}{2025/2026} % [cite: 379]
\newcommand{\datum}{9.1.2026} % [cite: 383]
\newcommand{\misto}{V Plzni} % [cite: 383]
...
\begin{titlepage}
    \begin{center}
        % 1. Název školy nahoře [cite: 1]
        \Large \textbf{\skola} \\ 
        \vspace{5cm} % Posun dolů (upravte podle potřeby)

        % 2. Maturitní práce [cite: 2]
        \Huge \textbf{\typPrace} \\ 
        \vspace{2cm} % Posun dolů
        
        % 3. Číslo a název projektu [cite: 3]
        % Spojíme kód práce a název do jednoho řádku, jak je v šabloně
        \LARGE \textbf{\kodPrace \ \ \nazevPrace} \\ 
        
    \end{center}

    \vfill % Vytlačí text níže až na samý spodek stránky

    % 4. Školní rok a Autor na jednom řádku [cite: 4, 5]
    % \noindent zruší odsazení odstavce, \hfill zarovná autora doprava
    \noindent
    \Large
    Školní rok \skolniRok \hfill \autor, \trida 
\end{titlepage}

% ================= PROHLÁŠENÍ =================
\newpage
\thispagestyle{empty}
\section*{PROHLÁŠENÍ} % [cite: 381]

% Text of the declaration from the PDF
Prohlašuji, že jsem tuto maturitní práci vypracoval samostatně a~veškeré zdroje uvedl v~seznamu použitých informačních zdrojů. % 

\vspace{2cm}

% Signatures for both authors
\noindent
\begin{minipage}{0.45\textwidth}
    \misto \ dne \datum \\ % [cite: 383]
    \vspace{1.5cm}
    \hrule
    \vspace{0.2cm}
    \centering \autor % [cite: 384]
\end{minipage}
...
Výstupní vektor z~ResNet-50 o~délce 2048 prvků je pro současné kvantové simulátory a~procesory příliš rozměrný. 
Bylo proto nutné jej redukovat na 64 prvků, aby odpovídal Hilbertovu prostoru 6 qubitů (\(2^{6} = 64\)) při amplitudovém kódování.
V~experimentální části byly porovnány tři přístupy: \textbf{PCA}, \textbf{LDA} a~\textbf{SelectKBest}, viz tabulka~\ref{tab:redukce_dim}.

\begin{table}[H]
    \centering
    \caption{Srovnání metod redukce dimenzionality pro hybridní model}
    \label{tab:redukce_dim}
...
\end{equation}
\myequation{Definice výstupní funkce VQC jako očekávané hodnoty operátoru měření \(M\) po unitární transformaci vstupního stavu \(|\psi_\mathbf{x}\rangle\), která slouží jako výstupní skóre klasifikátoru.}
...
\end{equation}
\myequation{Syrový výstup kvantového klasifikátoru \(y_{\text{raw}}\) definovaný jako střední hodnota operátoru \(Z_k\) pro stav \(|\psi_{\boldsymbol{\theta}}(\mathbf{x})\rangle\) v intervalu \([-1, 1]\).}
...
\end{equation}
\myequation{Lineární mapování syrového výstupu \(y_{\text{raw}}\) na pravděpodobnost pneumonie \(p_{\text{pneumonie}} \in [0,1]\), používanou pro následné binární rozhodnutí.}
...
\begin{itemize}
  ...
  \end{equation}
  \myequation{Výstup kvantového klasifikátoru VQC jako očekávaná hodnota Pauliho $Z$ operátoru.}
  ...
  \begin{equation}
    ...
    \end{cases}
  \end{equation}
  \myequation{Převedení binárních labelů na cílové hodnoty v~intervalu $[-1,1]$ pro trénink VQC.}
  a~používá se vážená střední kvadratická chyba
  \begin{equation}
    L_{\mathrm{QMSE}} = \frac{1}{N} \sum_{i=1}^{N} w(t_i)\,\bigl(z_i - t_i\bigr)^2.
  \end{equation}
  \myequation{Vážená střední kvadratická chyba \(L_{\mathrm{QMSE}}\) pro kvantový klasifikátor s třídními vahami \(w(t_i)\).}
  ...
  \begin{equation}
    L_{\mathrm{WBCE}} = -\frac{1}{N} \sum_{i=1}^{N}
      ...
      \bigr],
  \end{equation}
  \myequation{Vážená binární cross-entropie \(L_{\mathrm{WBCE}}\) používaná pro trénink klasického baseline modelu nad 64rozměrnými příznaky z~PCA.}
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
\myequation{Plán učení \(\eta(e)\) s lineárním warm-upem do epochy \(E_{\mathrm{warmup}}\) a následným kosinovým průběhem mezi \(\eta_0\) a \(\eta_{\min}\).}
